...
import type { DateRangeRowData, RowStatus } from '../lib/productionUtils';
...
export function ProductionDataGrid({ data, onUpdate, isFullTreatment, editMode = true }: ProductionDataGridProps) {
  ...
  useEffect(() => {
    ...
    data.forEach((row, i) => map.set(row.date, i));
  ...
  const onCellValueChanged = useCallback((event: CellValueChangedEvent) => {
    ...
    const rowIndex = indexMapRef.current.get(event.data.date);
    if (rowIndex !== undefined) {
      currentOnUpdate(rowIndex, event.colDef.field as keyof DateRangeRowData, event.newValue);
    }
  }, []);
          ...
          try {
            ...
            cells.forEach((cellValue, colOffset) => {
              try {
                ...
                const fieldName = colDef.field as keyof DateRangeRowData;
                rowNode.setDataValue(targetCol.getColId(), parsedValue);

                const rowIndex = indexMapRef.current.get(rowNode.data.date);
