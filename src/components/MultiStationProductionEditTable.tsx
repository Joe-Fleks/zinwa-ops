...
import type { RowData } from '../lib/productionUtils';

export type MultiStationProductionEditTableRef = FETRef;

interface MultiStationProductionEditTableProps {
  data: RowData[];
...
const STATION_LABEL = { header: 'Station', field: 'station_name', minWidth: 180 };

const rowKeyFn = (row: Record<string, any>) => `${row.station_id}-${row.date}`;

function MultiStationProductionEditTableComponent(
  { data, onUpdate, isFullTreatment, onUndoRedoStateChange }: MultiStationProductionEditTableProps,
  ref: React.Ref<MultiStationProductionEditTableRef>
) {
  const columns = isFullTreatment ? FT_COLUMNS : BH_COLUMNS;

  const handleUpdate = useCallback(
    (index: number, field: string, value: any) => onUpdate(index, field as keyof RowData, value),
    [onUpdate]
  );

  const sourceId = useMemo(() => {
    if (data.length === 0) return '';
    return `ms-${data[0]?.date}-${data.length}`;
  }, [data.length > 0 ? data[0]?.date : '', data.length]);
...
export const MultiStationProductionEditTable = forwardRef(MultiStationProductionEditTableComponent);
