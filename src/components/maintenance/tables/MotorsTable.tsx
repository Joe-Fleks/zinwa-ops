...
import {
  MotorRow, StationOption, MOTOR_TYPES, MOTOR_USES, DUTY_STATUSES, CONDITIONS, PHASES, ENCLOSURE_TYPES, POLES_OPTIONS,
...
interface Props {
  ...
  onDelete: (row: MotorRow) => void;
}

export default function MotorsTable({ rows, stations, editing, editRows, onCellChange, onDelete }: Props) {
  ...
  const headers = ['Station', 'Tag #', 'Manufacturer', 'Model', 'S/N', 'Type', 'Use', 'Duty', 'kW', 'HP', 'Voltage (V)', 'Amps (A)', 'RPM', 'Poles', 'Shaft (mm)', 'Phase', 'Enclosure', 'Installed', 'Life (yrs)', 'Expiry', 'Condition', 'Notes'];
          ...
          {(rows as MotorRow[]).map(row => {
            ...
            return (
              <tr key={row.id || `new_${realIdx}`} className={`border-b border-gray-100 ${row._isNew ? 'bg-blue-50/50' : row._isDirty ? 'bg-amber-50/50' : 'hover:bg-gray-50'}`}>
                ...
                <td className="px-1 py-0.5">
                  <select value={row.motor_type} onChange={e => onCellChange(realIdx, 'motor_type', e.target.value)} className={selectCls} style={{ minWidth: 100 }}>
                    <option value="">Select...</option>
                    {MOTOR_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </td>
                <td className="px-1 py-0.5">
                  <select value={row.motor_use} onChange={e => onCellChange(realIdx, 'motor_use', e.target.value)} className={selectCls} style={{ minWidth: 100 }}>
                    <option value="">Select...</option>
                    {MOTOR_USES.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </td>
                <td className="px-1 py-0.5">
                ...
                <td className="px-1 py-0.5"><input type="number" value={row.kw_rating || ''} onChange={e => onCellChange(realIdx, 'kw_rating', e.target.value ? parseFloat(e.target.value) : 0)} className={inputCls} style={{ minWidth: 55 }} /></td>
                <td className="px-1 py-0.5"><input type="number" value={row.hp_rating || ''} onChange={e => onCellChange(realIdx, 'hp_rating', e.target.value ? parseFloat(e.target.value) : 0)} className={inputCls} style={{ minWidth: 55 }} /></td>
                ...
                <td className="px-1 py-0.5">
                  <select value={row.poles || ''} onChange={e => onCellChange(realIdx, 'poles', e.target.value ? parseInt(e.target.value) : null)} className={selectCls} style={{ minWidth: 55 }}>
                    <option value="">--</option>
                    {POLES_OPTIONS.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </td>
                ...
                <td className="px-1 py-0.5">
                  <select value={row.enclosure_type} onChange={e => onCellChange(realIdx, 'enclosure_type', e.target.value)} className={selectCls} style={{ minWidth: 70 }}>
                    <option value="">--</option>
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={row.id} className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/30 transition-colors`}>
            ...
            <td className="px-3 py-2 text-gray-700">{row.motor_type || '--'}</td>
            <td className="px-3 py-2 text-gray-700">{row.motor_use || '--'}</td>
            ...
            <td className="px-3 py-2 text-right text-gray-700 font-medium">{row.kw_rating || '--'}</td>
            <td className="px-3 py-2 text-right text-gray-700 font-medium">{row.hp_rating || '--'}</td>
            ...
            <td className="px-3 py-2 text-center">{row.poles ? <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-indigo-50 text-indigo-700">{row.poles}-pole</span> : '--'}</td>
