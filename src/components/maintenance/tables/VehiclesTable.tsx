...
export default function VehiclesTable({ rows, serviceCentres, editing, editRows, onCellChange, onDelete }: Props) {
  ...
  const headers = [
    ...
    'Status', 'ZINARA Exp.', 'Insurance Exp.', 'Fitness Exp.',
          ...
          {(rows as VehicleRow[]).map(row => {
            ...
            return (
              <tr key={row.id || `new_${realIdx}`} className={`border-b border-gray-100 ${row._isNew ? 'bg-blue-50/50' : row._isDirty ? 'bg-amber-50/50' : 'hover:bg-gray-50'}`}>
                ...
                <td className="px-1 py-0.5"><input type="date" value={row.insurance_expiry} onChange={e => onCellChange(realIdx, 'insurance_expiry', e.target.value)} className={inputCls} style={{ minWidth: 115 }} /></td>
                <td className="px-1 py-0.5"><input type="date" value={row.fitness_expiry} onChange={e => onCellChange(realIdx, 'fitness_expiry', e.target.value)} className={inputCls} style={{ minWidth: 115 }} /></td>
                <td className="px-1 py-0.5">
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={row.id} className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/30 transition-colors`}>
            ...
            <td className="px-3 py-2 whitespace-nowrap">{expiryDateBadge(row.insurance_expiry)}</td>
            <td className="px-3 py-2 whitespace-nowrap">{expiryDateBadge(row.fitness_expiry)}</td>
