...
function buildContent(d: QuarterlyReportData): string {
  ...
  if (rw.length > 0) {
    ...
    parts.push(trow([
      ...
      { text: 'Target (ML)', shade: '1A3A5C', align: 'right' },
      { text: 'Sales (ML)', shade: '1A3A5C', align: 'right' },
      { text: 'Sales vs Target', shade: '1A3A5C', align: 'right' },
    ], true));
    ...
    parts.push(trow([
      ...
      { text: fmt(totTarget, 2), shade: 'E8EEF5', bold: true, align: 'right' },
      { text: fmt(totSales, 2), shade: 'E8EEF5', bold: true, align: 'right' },
      { text: totSvt !== null ? fmt(totSvt, 1) + '%' : 'N/A', shade: 'E8EEF5', bold: true, align: 'right' },
    ]));
    parts.push('</w:tbl>');
  } else {
