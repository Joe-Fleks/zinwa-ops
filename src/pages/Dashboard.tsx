import { AlertTriangle, ExternalLink, Fuel, FlaskConical, ClipboardList, Plus, Pencil, Check, X, FileText, Download } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const loadCustomAlerts = async () => {
    let query = supabase
      ...
      .select('id, subtitle, body, created_by')
      .order('created_at', { ascending: true });

    if (serviceCentreId) {
      query = query.eq('service_centre_id', serviceCentreId);
    }
  ...
  const handleAddCustomAlert = async () => {
    ...
    if (!error && data) {
      setCustomAlerts(prev => [...prev, data]);
      setNewSubtitle('');
      setNewBody('');
  ...
  const handleEditCustomAlert = async (id: string) => {
    const { error } = await supabase
      .from('custom_followup_alerts')
      .update({ subtitle: editSubtitle.trim(), body: editBody.trim(), updated_at: new Date().toISOString() })
      .eq('id', id);

    if (!error) {
      setCustomAlerts(prev =>
        prev.map(a => a.id === id ? { ...a, subtitle: editSubtitle.trim(), body: editBody.trim() } : a)
      );
      setEditingId(null);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col p-6 gap-6 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-5 border border-gray-200 flex flex-col overflow-hidden">
          <h2 className="text-lg font-semibold text-gray-900 mb-4 flex-shrink-0">Daily Alerts</h2>
          {loading ? (
            ...
            <div className="space-y-3 overflow-y-auto thin-scrollbar flex-1 pr-1">
                  ...
                  <div key={report.id} className="bg-sky-50 border-2 border-sky-400 rounded-lg px-3 py-2.5">
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sky-900 uppercase" style={{ fontSize: '11px' }}>
                          Weekly Report Ready
                        </p>
                        <p className="text-sm font-semibold text-sky-800 mt-0.5">
                          {reportTypeLbl} Report — Week {report.week_number}, {report.year}
                        </p>
                ...
                return (
                  <div key={report.id} className="bg-teal-50 border-2 border-teal-400 rounded-lg px-3 py-2.5">
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-teal-900 uppercase" style={{ fontSize: '11px' }}>
                          Monthly Report Ready
                        </p>
                        <p className="text-sm font-semibold text-teal-800 mt-0.5">
                          {monthLabel} {report.year} Operations Report
                        </p>
                        <p className="text-xs text-teal-700 mt-0.5">
                          {report.report_data?.production?.stationCount ?? 0} stations  ·  {report.report_data?.completionPct ?? 0}% data coverage
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  {nonFunctionalStats.savedPercentage < 100 && (
                    <div className="py-0.5">
                      ...
                      <p className="text-sm text-gray-700">
                        {nonFunctionalStats.savedRecordsCount} out of {nonFunctionalStats.totalStations} stations updated: {Math.round(nonFunctionalStats.savedPercentage)}%
                      </p>
                    </div>
                  )}
                      ...
                      <div className="py-0.5">
                        ...
                        <div className="flex items-center justify-between gap-3 mt-0.5">
                          <p className="text-sm text-red-700">
                            {nonFunctionalStats.unmetDemandPct}% of daily demand
                          </p>
                          <Link
                            to={`/sc/${scId}/maintenance?tab=non-functional`}
                            className="px-2.5 py-1 bg-red-700 text-white text-xs font-medium rounded hover:bg-red-800 transition-colors flex items-center gap-1.5 whitespace-nowrap"
                          >
                            More Details
                ...
                <div className="bg-gray-50 border border-gray-300 rounded-lg px-3 py-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        to={`/sc/${scId}/stock-control?tab=fuel&fuel=diesel`}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        to={`/sc/${scId}/stock-control?tab=fuel&fuel=petrol`}
                        className="text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 whitespace-nowrap"
                      >
                        See control card
                        <ExternalLink className="w-3 h-3" />
                        ...
                        <div key={section.key}>
                          ...
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <Link
                              ...
                              className={`text-xs font-medium rounded px-2 py-0.5 flex items-center gap-1 whitespace-nowrap transition-colors ${
                                hasCritical
                ...
                <div className="bg-gray-50 border border-gray-300 rounded-lg px-3 py-2">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-gray-800 uppercase" style={{ fontSize: '11px' }}>Follow-ups</span>
                    </div>
                    <button
                      onClick={() => { setShowAddForm(true); setNewSubtitle(''); setNewBody(''); }}
                  ...
                  <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between gap-3 mt-0.5">
                          <p className="text-sm text-gray-800">
                            <span className="text-amber-700">
                              {pendingSummarySheets.pendingCount}/{pendingSummarySheets.totalCount} stations
                            </span>
                            {' '}
                            <span className="text-gray-500 text-xs">
                              — {pendingSummarySheets.totalClientWeight > 0
                                ? Math.round((pendingSummarySheets.pendingClientWeight / pendingSummarySheets.totalClientWeight) * 100)
                                : Math.round((pendingSummarySheets.pendingCount / pendingSummarySheets.totalCount) * 100)}% outstanding
                            </span>
                          </p>
                          <Link
                            to={`/sc/${scId}/clearwater?tab=sales&filter=pending`}
                            className="text-xs text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 whitespace-nowrap font-medium"
                          >
                            See Details
                            <ExternalLink className="w-3 h-3" />
                          </Link>
                    ...
                    {customAlerts.map((alert) => (
                      <div key={alert.id} className="border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                        {editingId === alert.id ? (
                          <div className="space-y-1.5">
                            ...
                            <div className="flex gap-1.5">
                              <button
                                onClick={() => handleEditCustomAlert(alert.id)}
                                className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded transition-colors"
                              >
                          ...
                          <>
                            {alert.subtitle && (
                              <p className="text-xs font-bold text-gray-700">{alert.subtitle}</p>
                            )}
                            {alert.body && (
                              <p className="text-sm text-gray-700 mt-0.5">{alert.body}</p>
                            )}
              ...
              {alerts.length === 0 && !nonFunctionalStats && !fuelBalances && !chemicalAlerts ? (
